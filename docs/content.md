...
All article data lives in a single file: **`src/game/articles.js`**.
...
`src/game/ads.js` (same pattern as `articles.js`) and import.
...
## Rewriting all copy for the theme

Once the LD theme drops, most of what's in `articles.js` will need
rewriting to match your interpretation. Keep the schema — just change
the strings. Nothing else in the codebase reads those specific
